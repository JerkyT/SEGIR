...
class PointMetaBaseEncoder(nn.Module):
    ...
    def forward_cls_feat_Feature_reduction(self, p0, f0=None):
        if hasattr(p0, 'keys'):
            p0, f0 = p0['pos'], p0.get('x', None)
        if f0 is None:
            f0 = p0.clone().transpose(1, 2).contiguous()
        for i in range(0, len(self.encoder)):
            pe = None
            p0, f0, pe = self.encoder[i]([p0, f0, pe])
            # torch.Size([24, 64, 512])
            # torch.Size([24, 128, 256])
            # torch.Size([24, 256, 128])
            # torch.Size([24, 512, 64])
            # torch.Size([24, 512, 1])
                        
            if i in [0, 4]:
                # 1. 计算 token importance (L2 范数)
                token_importance = f0.norm(p=2, dim=-1)  # [B, 128]

                # 2. 选 top-k
                k = random.uniform(0.01, 0.2)
                topk = torch.topk(token_importance, int(k * f0.size(1)), dim=1).indices  # [B, k]
