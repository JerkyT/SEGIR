...
class PointNet2Encoder(nn.Module):
    ...
    def forward_cls_feat_Feature_reduction(self, xyz, features=None):
        if hasattr(xyz, 'keys'):
            xyz, features = xyz['pos'], xyz['x']
        if features is None:
            features = xyz.clone().transpose(1, 2).contiguous()
        ...
        for i in range(len(self.SA_modules)):
            ...
            if i in [0, 4]:
                # 1. 计算 token importance (L2 范数)
                token_importance = features.norm(p=2, dim=-1)  # [B, 128]

                # 2. 选 top-k
                k = random.uniform(0.01, 0.2)
                topk = torch.topk(token_importance, int(k * features.size(1)), dim=1).indices  # [B, k]

                # 3. 构造 mask: [B, 128]
                mask = torch.zeros_like(token_importance).scatter(1, topk, 1.0)  # [B, 128]

                # 4. 扩展为 [B, 128, 1] 再广播
                mask = mask.unsqueeze(-1)  # [B, 128, 1]

                # 5. 如果要用随机衰减
                decay = torch.rand_like(mask) * 0.2  # [B, 128, 1] ∈ [0, 0.2]
                features = features * (1 - mask * decay)  # 广播正确 ✅
        return features.squeeze(-1)

            # 0 torch.Size([24, 128, 512])
            # 1 torch.Size([24, 256, 128])
            # 2 torch.Size([24, 1024, 1])
            # 3 torch.Size([24, 128, 512])
            # 4 torch.Size([24, 256, 128])
            # 5 torch.Size([24, 1024, 1])
