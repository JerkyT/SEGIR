...
class BaseCls(nn.Module):
    ...
    def forward(self, data, cut = False):
        ##
        if cut:
            global_feat = self.encoder.forward_cls_feat_Feature_reduction(data)
        ##
        elif get_return_count(self.encoder.forward_cls_feat) == 1 or get_return_count(self.encoder.forward_cls_feat) == None:
            global_feat = self.encoder.forward_cls_feat(data)
        else:
            global_feat, _ = self.encoder.forward_cls_feat(data)
    ...
    def get_logits_loss(self, data, gt, cfg = None, epoch = None):
        if cfg.criterion_args.NAME in ['PSKD']:
            logits = self.forward(data, cfg.cut)
        ...
        if not 'posc' in data:
            ...
            logits1 = self.forward(data_input, cfg.cut)
...
class DistillCls(BaseCls):
    ...
    def get_logits_loss(self, data, gt):
        logits, dist_logits = self.forward(data, cfg)
...
class BaseCls_ASE(nn.Module):
    ...
    def get_logits_loss(self, data, gt, cfg = None, epoch = None):
        ...
        return logits, self.criterion(logits, gt.long()) * 0.5 + loss_extra * 0.5
