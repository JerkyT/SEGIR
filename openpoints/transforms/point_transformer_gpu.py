...
@DataTransforms.register_module()
class Graph_Domain_Geometry_Enhancement(object):
    def __init__(self, **kwargs):
        pass

    def __call__(self, data):
        # a = random.uniform(0.0, 1) #random.uniform(0.0, 0.3)
        a = 0.3
        point_cloud, point_sk, v_real, v_real_sk, fv, fv_sk = data['pos'], data['sk'], data['v_real'], data['v_real_sk'], data['fv'], data['fv_sk']
        # v_real, fv, v_real_sk, fv_sk = None, None, None, None
        x, x_, x_or = GFT(point_cloud, v_real = v_real, x_ = fv)
        xsk, xsk_, xsk_or = GFT(point_sk, v_real = v_real_sk, x_ = fv_sk)
        new_s = x_or

        ###
        xsk_or[100:900] = normalize_array_torch(xsk_or[100:900]) - 0.5
        new_s[100::] = new_s[100::] * (a * xsk_or[100::] + 1)
        ###

        adv_point = iGFT(point_cloud, new_s, v_real = v_real)
